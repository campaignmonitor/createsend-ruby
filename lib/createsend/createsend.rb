...
module CreateSend
  ...
  class CreateSend
    ...
    # Specify cert authority file for cert validation (with default)
    @@pem_file = File.expand_path(File.join(File.dirname(__FILE__), 'cacert.pem'))
    ssl_ca_file @@pem_file
    def self.pem(pem_file)
      @@pem_file = pem_file
      ssl_ca_file @@pem_file
    end
