...
class SubscriberTest < Test::Unit::TestCase
  context "when an api caller is authenticated" do
    ...
    should "get a subscriber by list id and email address" do
      ...
      stub_get(@auth_options, "subscribers/#{@list_id}.json?email=#{CGI.escape(email)}", "subscriber_details.json")
    ...
    should "update a subscriber with custom fields" do
      ...
      stub_put(@auth_options, "subscribers/#{@list_id}.json?email=#{CGI.escape(email)}", nil)
      custom_fields = [ { :Key => 'website', :Value => 'http://example.com/' } ]
    ...
    should "import many subscribers at once" do
      stub_post(@auth_options, "subscribers/#{@list_id}/import.json", "import_subscribers.json")
      subscribers = [
    ...
    should "import many subscribers at once with partial success" do
      ...
      stub_post(@auth_options, "subscribers/#{@list_id}/import.json", "import_subscribers_partial_success.json", 400)
      subscribers = [
    ...
    should "unsubscribe a subscriber" do
      stub_post(@auth_options, "subscribers/#{@subscriber.list_id}/unsubscribe.json", nil)
      @subscriber.unsubscribe
    end
    
    should "get a subscriber's history" do
      stub_get(@auth_options, "subscribers/#{@subscriber.list_id}/history.json?email=#{CGI.escape(@subscriber.email_address)}", "subscriber_history.json")
    ...
    should "delete a subscriber" do
      stub_delete(@auth_options, "subscribers/#{@subscriber.list_id}.json?email=#{CGI.escape(@subscriber.email_address)}", nil)
      @subscriber.delete
    end
  end
end